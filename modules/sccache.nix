...
in {
		...
		environment.sessionVariables.RUSTC_WRAPPER = "${pkgs.sccache}/bin/sccache";
                environment.etc."sccache/scheduler.conf" = mkIf cfg.scheduler {mode = "0644"; text = ''
...
token = "${cfg.token}"
                ...
                environment.etc."sccache/server.conf" = mkIf cfg.server {mode = "0644";  text = ''
...
token = "${cfg.token}" 
               '';};
                ...
                home-manager.users."${user}".xdg.configFile."sccache/config" = mkIf cfg.client {text = ''
[dist]
# The URL used to connect to the scheduler (should use https, given an ideal
# setup of a HTTPS server in front of the scheduler)
scheduler_url = "${cfg.sched_url}"
